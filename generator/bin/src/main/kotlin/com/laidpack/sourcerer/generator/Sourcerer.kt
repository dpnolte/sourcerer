...
class Sourcerer(
    ...
    fun generateFactoriesForClass(indexedClass: IndexedClass) {
        ...
        if (result != null) {
            ...
            generateFactoryFiles(env.stubAppDir, result)
            println("----")
            println("Generated files for ${result.targetClassName.simpleName}")
        }
        println("=================================")
    }

    fun analyzeSourceOfClass(classInfo: ClassInfo, checkResolvementOfSuperClasses: Boolean = true): SourcererResult {
        if (checkResolvementOfSuperClasses) {
        ...
        if (attributes.isNotEmpty()) {
            ...
            interpreterFactories.forEach { factory ->
                val interpreter = factory.create(classInfo, typedArrayInfo, attributeManager)
            ...
            codeBlocks = typePhilosopher.reflectOnCodeBlockSociety(resolvedAttributes, setters)
    ...
    private fun ensureSuperClassesHaveResolvedAttributes(classInfo: ClassInfo) {
        ...
        for (superClassInfo in superClassesInfo) {
            if (!superClassInfo.hasResolvedAttributes) {
                ...
                analyzeSourceOfClass(superClassInfo, checkResolvementOfSuperClasses = false)
            }
        }
    }
