                    ...
                    if (cu != null && cu.packageDeclaration.isPresent) {
                        ...
                        for (classOrInterfaceDeclaration in classOrInterfaceDeclarations) {
                            ...
                            if (isClassEligibleForIndexing(targetClassName, hashCode, canBeWidget)) {
                                ...
                                var widgetLabel = ""
                                if (canBeWidget) {
                                    if (this.canClassBeWidgetView((classOrInterfaceDeclaration))) {
                                        val widget = widgetRegistry.add(indexedClass, indexedFile)
                                        ClassRegistry.assignWidgetAsBeingViewClass(indexedClass, widget)
                                        widgetLabel = " + widget view"
                                    } else if (this.canClassBeWidgetLayoutParams(classOrInterfaceDeclaration, widgetParser, url)) {
                                        val widget = widgetRegistry.add(indexedClass, indexedFile)
                                        ClassRegistry.assignWidgetAsBeingLayoutParamsClass(indexedClass, widget)
